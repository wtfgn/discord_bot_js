...
export const loadCommands = async (client = null) => {
	...
	const files = await fg('./src/commands/**/index.js', {
		ignore: './src/commands/**/sub_commands/**/index.js',
	});
...
export const loadSubCommands = async (commandName) => {
	...
	const files = await fg(
		`./src/commands/**/${commandName}/sub_commands/**/index.js`,
	);
	for (const file of files) {
		// Import sub command
	...
	logger.info(
		`Successfully loaded ${subCommands.size} sub commands - [ ${subCommands
			.map((subCommand) => subCommand.data.name)
			.join(', ')} ] for command ${commandName}`,
	);
	return subCommands;
};
...
export const deleteGuildCommands = async (guildID = null) => {
	...
	try {
		if (!guildID) {
			logger.error(
				'Guild ID is not defined, please define it in .env file or pass it as an argument',
			);
			return;
		}
