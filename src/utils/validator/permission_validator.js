...
export const cannotJoinVoiceOrTalk = async (interaction) => {
	...
	if (!voiceChannel.joinable || !voiceChannel.speakable) {
		...
		logger.debug(
			`User <${interaction.user.username}> tried to use <${interaction.commandName}> command without the bot having the correct permissions`,
		);
		return true;
	}
