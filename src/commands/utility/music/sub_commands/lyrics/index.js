...
export const execute = async (interaction) => {
	...
	if (!lyricsResult && geniusSearchQuery.length > 20) {
		lyricsResult = await genius
			.search(geniusSearchQuery.slice(0, 20))
			.catch(() => null);
	}
	if (!lyricsResult && geniusSearchQuery.length > 10) {
		lyricsResult = await genius
			.search(geniusSearchQuery.slice(0, 10))
			.catch(() => null);
	}
	...
	if (searchResult && lyricsResult) {
		...
		if (
			...
			logger.debug(
				`User <${interaction.user.username}> tried to use <${interaction.commandName}> command with no matching artist`,
			);
	...
	if (!lyricsResult || !lyricsResult.lyrics) {
		logger.debug(
			`User <${interaction.user.username}> tried to use <${interaction.commandName}> command with no lyrics found`,
		);
	...
	if (lyricsResult.lyrics.length > 3800) {
		logger.debug(
			`User <${interaction.user.username}> tried to use <${interaction.commandName}> command with too long lyrics`,
		);
		const messageCount = Math.ceil(lyricsResult.lyrics.length / 3800);

		for (let i = 0; i < messageCount; i++) {
			logger.debug(
				`User <${interaction.user.username}> tried to use <${
					interaction.commandName
				}> command with too long lyrics, sending message ${
					i + 1
				}/${messageCount}`,
			);
	...
	logger.debug(
		`User <${interaction.user.username}> used <${interaction.commandName}> command and got lyrics`,
	);
