...
export const execute = async (interaction) => {
	...
	if (!searchResult || !searchResult.tracks.length) {
		logger.debug(
			`User <${interaction.user.username}> tried to use <${interaction.commandName}> command with no search results`,
		);
	...
	if (
		(searchResult.playlist && searchResult.tracks.length) >
		playerOptions.maxQueueSize - queueSize
	) {
		logger.debug(
			`User <${interaction.user.username}> tried to use <${interaction.commandName}> command with too many tracks in playlist`,
		);
	...
	try {
		...
		if (err.message.includes('The following content may contain')) {
			logger.debug(
				`User <${interaction.user.username}> tried to use <${interaction.commandName}> command with graphic or sensitive track`,
			);
		...
		if (
			...
			logger.debug(
				`User <${interaction.user.username}> tried to use <${interaction.commandName}> command with invalid track`,
			);
		...
		if (err.message === 'Cancelled') {
			logger.debug(
				`User <${interaction.user.username}> tried to use <${interaction.commandName}> command but cancelled`,
			);
	...
	if (!queue) {
		logger.debug(
			`User <${interaction.user.username}> tried to use <${interaction.commandName}> command but failed to add track to queue`,
		);
	...
	if (searchResult.playlist && searchResult.tracks.length > 1) {
		logger.debug(
			`User <${interaction.user.username}> used <${interaction.commandName}> command and added a playlist to queue`,
		);
	...
	if (queue.currentTrack === track && queue.tracks.data.length === 0) {
		logger.debug(
			`User <${interaction.user.username}> used <${interaction.commandName}> command and started playing track <${track.title}>`,
		);
