...
export const execute = async (interaction) => {
	...
	if (weatherData.cod === '404') {
		logger.debug(
			`User <${interaction.user.username}> tried to get weather data for city ${city} but it was not found`,
		);
	...
	if (weatherData.cod !== 200) {
		logger.debug(
			`User <${interaction.user.username}> tried to get weather data for city ${city} but something went wrong`,
		);
		return interaction.reply({
	...
	const embed = new EmbedBuilder()
		...
		.addFields({
			...
			value: `*Current*: ${weatherData.main.temp}°C
		...
		.addFields({
			name: ':dash:Wind',
			value: `*Speed*: ${weatherData.wind.speed} m/s
		...
		.addFields({
			name: ':cloud:Clouds',
			value: `*Cloudiness*: ${weatherData.clouds.all}%`,
			inline: true,
		})
		.addFields({
			name: ':sunny:Sun',
			value: `*:sunrise:Sunrise*: ${dayjs
	...
	if (weatherData.rain) {
		embed.addFields({
			...
			value: `*1h*: ${weatherData.rain['1h'] ? weatherData.rain['1h'] : '0'} mm
	...
	if (weatherData.snow) {
		embed.addFields({
			name: ':snowflake:Snow',
			value: `*1h*: ${weatherData.snow['1h'] ? weatherData.snow['1h'] : '0'} mm
