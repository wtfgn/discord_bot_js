...
export const execute = async (interaction) => {
	...
	if (!alarm) {
		logger.debug(`User <${interaction.user.username}> tried to remove an alarm with id <${id}> but it did not exist`);
	...
	if (alarm.userId !== interaction.member.id) {
		logger.debug(`User <${interaction.user.username}> tried to remove an alarm with id <${id}> but they were not the owner`);
		return interaction.reply({
