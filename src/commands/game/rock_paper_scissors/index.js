...
export const execute = async (interaction) => {
	...
	if (opponent.id === interaction.user.id) {
		...
		logger.debug(
			`User <${interaction.user.username}> tried to play rock paper scissors against themselves`,
		);
	...
	const filter = (i) => {
		if (i.user.id !== interaction.user.id && i.user.id !== opponent.id) {
			...
			logger.debug(
				`User <${i.user.username}> tried to interact with rock paper scissors button but they are not allowed`,
			);
		...
		if (
			(i.user.id === interaction.user.id && playerMove) ||
			...
			logger.debug(
				`User <${i.user.username}> tried to interact with rock paper scissors button but they have already selected a move`,
			);
			i.reply({ embeds: [warningEmbed], ephemeral: true });
			return false;
		}
	...
	collector.on('collect', async (componentInteraction) => {
		...
		if (playerMove && opponentMove) {
			...
			embed.setFooter({
				text: `Round ${
					currentRound > rounds ? currentRound - 1 : currentRound
	...
	collector.on('end', async (collected, reason) => {
		if (reason === 'time') {
			logger.debug(
				`User <${interaction.user.username}> finished playing rock paper scissors against <${opponent.username}> because the game timed out`,
			);
			embed.addFields({
				name: 'Result',
				value:
					'The game has ended because no one selected a move in 60 seconds!',
			});
		}
		...
		logger.debug(
			`User <${interaction.user.username}> finished playing rock paper scissors against <${opponent.username}>`,
		);
