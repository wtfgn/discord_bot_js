...
export const execute = async (member) => {
	logger.debug(
		`User <${member.user.username}> joined guild <${member.guild.name}>`,
	);
	const guild = await memberCountGuilds.findOne({
		where: { guildId: member.guild.id },
	});
	const displayChannel = member.guild.channels.cache.get(
